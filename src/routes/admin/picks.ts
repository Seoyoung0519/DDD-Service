...
import {
  ...
  updatePick,
  deletePick,
} from '../../repositories/admin/pickRepository';

const router = Router();

router.get('/', async (_req: Request, res: Response) => {
  try {
    const data = await listPicks(false);
...
router.post('/', async (req: Request, res: Response) => {
  try {
    const { title, description, book_isbn, cover_image_url, sort_order, is_active } = req.body as {
      title?: string;
...
router.patch('/:id', async (req: Request, res: Response) => {
  try {
    const data = await updatePick(req.params.id, req.body);
    if (!data) {
      return res.status(404).json({ success: false, message: 'Pick을 찾을 수 없습니다.' });
    }
    return res.json({ success: true, data });
...
router.delete('/:id', async (req: Request, res: Response) => {
  try {
    const existing = await getPickById(req.params.id);
    if (!existing) {
      return res.status(404).json({ success: false, message: 'Pick을 찾을 수 없습니다.' });
    }
    await deletePick(req.params.id);
